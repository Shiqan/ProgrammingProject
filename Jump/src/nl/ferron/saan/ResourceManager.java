...
public class ResourceManager extends Object {
	...
	public void setup(final Engine pEngine, final Context pContext, int id,
	...
	private void loadGameTextures() {
		...
		mGameAtlas = new BuildableBitmapTextureAtlas(
				mEngine.getTextureManager(), 1024, 1536);

		mBackgroundRegion = BitmapTextureAtlasTextureRegionFactory
				.createFromAsset(mGameAtlas, mContext, "bg.png");
		mBoxRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(
				mGameAtlas, mContext, "square.png");
		mCompleteRegion = mBoxRegion;
		mFloor1Region = BitmapTextureAtlasTextureRegionFactory.createFromAsset(
				mGameAtlas, mContext, "floor1.png");
		mFloor2Region = BitmapTextureAtlasTextureRegionFactory.createFromAsset(
				mGameAtlas, mContext, "floor2.png");
		mPlayerRegion = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(mGameAtlas, mContext, "player.png", 6, 1);
		mHandRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(
				mGameAtlas, mContext, "hand.png");
		mClingRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(
				mGameAtlas, mContext, "cling.png");
	...
	private void loadButtonTextures() {
		...
		mButtonAtlas = new BuildableBitmapTextureAtlas(
				mEngine.getTextureManager(), 1420, 340);

		mMenuButtonRegion = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(mButtonAtlas, mContext, "menu.png", 2, 1);
		mPauseButtonRegion = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(mButtonAtlas, mContext, "pause.png", 2, 1);
		mResumeButtonRegion = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(mButtonAtlas, mContext, "resume.png", 2,
						1);
		mRestartButtonRegion = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(mButtonAtlas, mContext, "restart.png", 2,
						1);
	...
	private void loadSounds() {
		...
		if (mOnDieSound == null) {
			try {
				mOnDieSound = SoundFactory.createSoundFromAsset(
						mEngine.getSoundManager(), mContext, "die.ogg");
		...
		if (mLevelId == 1) {
			if (mGameMusic == null) {
				try {
					mGameMusic = MusicFactory
							.createMusicFromAsset(mEngine.getMusicManager(),
									mContext, "vanguard.mp3");
				} catch (final IOException e) {
		...
		if (mOnVictorySound == null) {
			try {
				mOnVictorySound = MusicFactory.createMusicFromAsset(
						mEngine.getMusicManager(), mContext, "victory.ogg");
			} catch (final IOException e) {
				Log.v("Sounds Load", "Exception:" + e.getMessage());
			}
		}
	...
	private void loadFonts() {
		...
		if (mFontSlimJoe == null) {
			mFontSlimJoe = FontFactory.createFromAsset(
					mEngine.getFontManager(), mEngine.getTextureManager(), 256,
					256, TextureOptions.BILINEAR, mContext.getAssets(),
					"slimjoe.otf", 32f, true, Color.WHITE_ARGB_PACKED_INT);
			mFontSlimJoe.load();
		}
		if (mFontBigJohn == null) {
			mFontBigJohn = FontFactory.createFromAsset(
					mEngine.getFontManager(), mEngine.getTextureManager(), 512,
					512, TextureOptions.BILINEAR, mContext.getAssets(),
					"bigjohn.otf", 52f, true, Color.WHITE_ARGB_PACKED_INT);
			mFontBigJohn.load();
		}
	}
