...
public abstract class Player extends AnimatedSprite {
	...
	public Player(float pX, float pY, Camera camera, PhysicsWorld physicsWorld) {
		super(pX, pY, ResourceManager.getInstance().mPlayerRegion,
	...
	private void createPhysics(final Camera camera, PhysicsWorld physicsWorld) {
		mBody = PhysicsFactory.createBoxBody(physicsWorld, this,
				BodyType.DynamicBody, PhysicsFactory.createFixtureDef(0, 0, 0));

		mBody.setUserData("player");
		mBody.setFixedRotation(true);

		physicsWorld.registerPhysicsConnector(new PhysicsConnector(this, mBody,
				true, false) {
			@Override
			public void onUpdate(float pSecondsElapsed) {
				...
				if (mMove) {
					mBody.setLinearVelocity(new Vector2(8, mBody
							.getLinearVelocity().y));
				}
			}
		});
