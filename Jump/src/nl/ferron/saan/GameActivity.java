...
/**
 * Activity to create and handle the game
 * @author Ferron
 */
...
public class GameActivity extends SimpleBaseGameActivity implements
		IOnSceneTouchListener, ConnectionCallbacks, OnConnectionFailedListener {
	...
	public EngineOptions onCreateEngineOptions() {
		...
		mActualScreenWidthInches = getResources().getDisplayMetrics().widthPixels
				/ getResources().getDisplayMetrics().xdpi;
		mActualScreenHeightInches = getResources().getDisplayMetrics().heightPixels
				/ getResources().getDisplayMetrics().ydpi;
		// Set the Camera's Width & Height according to the device with which
		// you design the game
		mCameraWidth = Math.round(Math.max(Math.min(DESIGN_SCREEN_WIDTH_PIXELS
				* (mActualScreenWidthInches / DESIGN_SCREEN_WIDTH_INCHES),
				MAX_WIDTH_PIXELS), MIN_WIDTH_PIXELS));
		mCameraHeight = Math
				.round(Math.max(
						Math.min(
								DESIGN_SCREEN_HEIGHT_PIXELS
										* (mActualScreenHeightInches / DESIGN_SCREEN_HEIGHT_INCHES),
	...
	protected void onCreateResources() {
		...
		ResourceManager.getInstance().setup(this.getEngine(),
				GameActivity.this, mId, mCameraWidth, mCameraHeight,
				mCameraWidth / DESIGN_SCREEN_WIDTH_PIXELS,
				mCameraHeight / DESIGN_SCREEN_HEIGHT_PIXELS);
	...
	protected Scene onCreateScene() {
		...
		// this.mGameScene.setBackground(new Background(0.09804f, 0.6274f,
		// 0.8784f));
		mGameScene.setBackground(new SpriteBackground(new Sprite(
				mCameraWidth / 2, mCameraHeight / 2, mCameraWidth,
				mCameraHeight, ResourceManager.mBackgroundRegion,
				ResourceManager.getInstance().mEngine
		...
		mGameScene.setScale(ResourceManager.getInstance().mCameraScaleFactorX,
				ResourceManager.getInstance().mCameraScaleFactorY);
		mGameScene.setPosition(0,
				ResourceManager.getInstance().mCameraHeight / 2f);
		mGameHud.setScaleCenter(0f, 0f);
		mGameHud.setScale(ResourceManager.getInstance().mCameraScaleFactorX,
				ResourceManager.getInstance().mCameraScaleFactorY);
		...
		pPauseButton
				.setPosition(
						DESIGN_SCREEN_WIDTH_PIXELS
								- ((pPauseButton.getWidth() * pPauseButton
										.getScaleX()) / 2f),
						DESIGN_SCREEN_HEIGHT_PIXELS
								- (pPauseButton.getHeight() * pPauseButton
										.getScaleY()) / 2f);
	...
	private void createPhysics() {
		mPhysicsWorld = new FixedStepPhysicsWorld(60, new Vector2(0, -28),
				false);
	...
	private void loadLevel(int levelID) {
				...
				.registerEntityLoader(new EntityLoader<SimpleLevelEntityLoaderData>(
					...
					public IEntity onLoadEntity(
						...
						if (type.equals(TAG_ENTITY_ATTRIBUTE_TYPE_VALUE_BOX)) {
							plevelObject = new Sprite(x, y,
									ResourceManager.mBoxRegion, ResourceManager
											.getInstance().mEngine
											.getVertexBufferObjectManager());

							boolean alpha = SAXUtils.getBooleanAttribute(
									pAttributes,
									TAG_ENTITY_ATTRIBUTE_TYPE_ALPHA_BOX, false);
							if (alpha) {
								plevelObject
										.registerEntityModifier(new LoopEntityModifier(
												new AlphaModifier(2f, 1.0f,
														0.0f)));
							}

							Body body = PhysicsFactory.createBoxBody(
									mPhysicsWorld, plevelObject,
									BodyType.StaticBody, FIXTURE_DEF);
							body.setUserData("box");
							mPhysicsWorld
									.registerPhysicsConnector(new PhysicsConnector(
											plevelObject, body, false, false));
							...
							mPlayer = new Player(x, y, mCamera, mPhysicsWorld) {
								...
								public void onDie() {
									if (!mPlayer.isIgnoreUpdate()) {
										...
										if (!UserDataManager.getInstance()
												.isSoundMuted()) {
											ResourceManager.mGameMusic.pause();
											ResourceManager.mOnDieSound.play();
										}

										explode((int) mPlayer.getX(),
												(int) mPlayer.getY());
							...
							plevelObject = mPlayer;
								...
								.equals(TAG_ENTITY_ATTRIBUTE_TYPE_VALUE_FLOOR1)) {
							int width = SAXUtils.getIntAttributeOrThrow(
									pAttributes,
									LevelConstants.TAG_LEVEL_ATTRIBUTE_WIDTH);
							int height = SAXUtils.getIntAttributeOrThrow(
									pAttributes,
									LevelConstants.TAG_LEVEL_ATTRIBUTE_HEIGHT);

							plevelObject = new Sprite(x + width / 2, y,
									ResourceManager.mFloor1Region,
									ResourceManager.getInstance().mEngine
											.getVertexBufferObjectManager());
							...
							mPhysicsWorld
								...
								.equals(TAG_ENTITY_ATTRIBUTE_TYPE_VALUE_FLOOR2)) {
							int width = SAXUtils.getIntAttributeOrThrow(
									pAttributes,
									LevelConstants.TAG_LEVEL_ATTRIBUTE_WIDTH);
							int height = SAXUtils.getIntAttributeOrThrow(
							...
							mPhysicsWorld
								...
								.equals(TAG_ENTITY_ATTRIBUTE_TYPE_VALUE_HAND)) {
							plevelObject = new Sprite(x, y,
									ResourceManager.mHandRegion,
									ResourceManager.getInstance().mEngine
											.getVertexBufferObjectManager());

							Text pTutorialText = new Text(0, 0,
									ResourceManager.mFontSlimJoe,
									"TOUCH TO JUMP", ResourceManager
											.getInstance().mEngine
											.getVertexBufferObjectManager());
							pTutorialText.setPosition(
									x - (pTutorialText.getWidth() / 4),
									mCamera.getHeight() / 3);
							mGameScene.attachChild(pTutorialText);

						} else if (type
								.equals(TAG_ENTITY_ATTRIBUTE_TYPE_VALUE_CLING)) {
							plevelObject = new Sprite(x, y,
									ResourceManager.mClingRegion,
									ResourceManager.getInstance().mEngine
											.getVertexBufferObjectManager());
							plevelObject
									.registerEntityModifier(new LoopEntityModifier(
											new ScaleModifier(1f, 0.1f, 0.3f)));

						} else if (type
								.equals(TAG_ENTITY_ATTRIBUTE_TYPE_VALUE_LEVEL_COMPLETE)) {
							plevelObject = new Sprite(x, y,
									ResourceManager.mCompleteRegion,
									ResourceManager.getInstance().mEngine
											.getVertexBufferObjectManager());
							Body body = PhysicsFactory.createBoxBody(
									mPhysicsWorld, plevelObject,
		...
		levelLoader.loadLevelFromAsset(this.getAssets(), "level/" + levelID
				+ ".lvl");
	}
		...
		ContactListener contactListener = new ContactListener() {
			public void beginContact(Contact contact) {
				...
				if (x1.getBody().getUserData() != null
					...
					// FLOOR 2
					if ((x1.getBody().getUserData().equals("floor2") && x2
							.getBody().getUserData().equals("player"))
							|| (x1.getBody().getUserData().equals("player") && x2
									.getBody().getUserData().equals("floor2"))) {
						mPlayer.onDie();
					}

					// BOX
					if (x2.getBody().getUserData().equals("player")
							&& x1.getBody().getUserData().equals("box")) {
						float x = x1.getBody().getPosition().x;
						int err = (int) (1 + (x1.getBody().getPosition().y / 15.5f));
						if ((x - err) > x2.getBody().getPosition().x
								|| x1.getBody().getPosition().y > x2.getBody()
										.getPosition().y) {
							mPlayer.onDie();
						}
					}
					...
					// LEVELCOMPLETE
					if ((x1.getBody().getUserData().equals("levelComplete") && x2
							.getBody().getUserData().equals("player"))
							|| (x1.getBody().getUserData().equals("player") && x2
									.getBody().getUserData()
									.equals("levelComplete"))) {

						if (ResourceManager.mOnVictorySound != null
								& !ResourceManager.mOnVictorySound.isPlaying()
	 ...
	 *            x position of particle system
	 * @param particleSpawnCenterY
	 *            y position of particle system
	 */
	private void explode(int particleSpawnCenterX, int particleSpawnCenterY) {
		...
		CircleParticleEmitter particleEmitter = new CircleParticleEmitter(
				particleSpawnCenterX, particleSpawnCenterY, particleSpawnRadius);
		...
		BatchedSpriteParticleSystem particleSystem = new BatchedSpriteParticleSystem(
				...
				ResourceManager.getInstance().mPlayerRegion,
				ResourceManager.getInstance().mEngine
						.getVertexBufferObjectManager());
	...
	/**
	 * Convert player x position to percentage and check for achievements and
	 * highscores
	 * 
	 * @param posX
	 ...
	 * 
	 * @param pLayerType
	 *            0 (paused), 1 (game over), 2 (completed)
	 */
	private void showLayer(int pLayerType) {
		...
		case 1:
			...
			ResourceManager.getInstance().mEngine
					.registerUpdateHandler(new TimerHandler(5f,
							new ITimerCallback() {
		...
		Rectangle pBackground = new Rectangle(mCameraWidth / 2,
				mCameraHeight / 2, mCameraWidth, mCameraHeight,
				ResourceManager.getInstance().mEngine
						.getVertexBufferObjectManager());
		...
		Text pLayerTitle = new Text(0, 0, ResourceManager.mFontBigJohn,
				pLayerTitleString,
				ResourceManager.getInstance().mEngine
						.getVertexBufferObjectManager());
		pLayerTitle.setPosition((pBackground.getWidth() / 2f),
				(pBackground.getHeight() - pLayerTitle.getHeight()));
		layer.attachChild(pLayerTitle);

		// Create the score text for the Layer
		Text pScoreText = new Text(0, 0, ResourceManager.mFontSlimJoe,
				calculateScore((int) mPlayer.getX()),
				ResourceManager.getInstance().mEngine
						.getVertexBufferObjectManager());
		pScoreText.setPosition(
				(((pBackground.getWidth() + pScoreText.getWidth()) / 2f) / 3f),
				((pBackground.getHeight() + pScoreText.getHeight()) / 2f));
		layer.attachChild(pScoreText);

		// Create a menu button
		ButtonSprite pMainMenuButton = new ButtonSprite(0, 0,
		...
		pMainMenuButton
				.setPosition(
						2f * ((pBackground.getWidth() + pMainMenuButton
								.getWidth()) / 3f),
		...
		ButtonSprite pRestartButton = new ButtonSprite(0, 0,
		...
		pRestartButton
				.setPosition(
						2f * ((pBackground.getWidth() + pRestartButton
								.getWidth()) / 3f),
						((pBackground.getHeight() + pRestartButton.getHeight()) / 2f)
								- pRestartButton.getHeight());
		...
		if (pCreateResumeButton) {
			ButtonSprite pResumeButton = new ButtonSprite(0, 0,
			...
			pResumeButton
					.setPosition(
							2f * ((pBackground.getWidth() + pResumeButton
									.getWidth()) / 3f),
							((pBackground.getHeight() + pResumeButton
									.getHeight()) / 2f)
									- 2f
									* pResumeButton.getHeight());
	...
	public void onResumeGame() {
		super.onResumeGame();
		if (!isSignedIn() & !MainActivity.mSignInFailed
				& !MainActivity.mSignOutClicked) {
	...
	public void onPauseGame() {
		...
		if (ResourceManager.mGameMusic != null
				&& ResourceManager.mGameMusic.isPlaying()) {
			ResourceManager.mGameMusic.pause();
		}
	 ...
	 *            activity context
	 * @param score
	 *            the percentage of the level that is cleared
	 */
			...
			if (mId == 1) {
				if (score > 50) {
					Games.Achievements.unlock(mGoogleApiClient,
							c.getString(R.string.achievement_halfway_level_1));
				}
				if (score == 100) {
					Games.Achievements
							.unlock(mGoogleApiClient,
									c.getString(R.string.achievement_completed_level_1));
				}

				// Push score to leaderbords
				Games.Leaderboards.submitScore(mGoogleApiClient,
						getString(R.string.leaderboard_level_1), score);
			} else if (mId == 2) {
				if (score > 50) {
					Games.Achievements.unlock(mGoogleApiClient,
							c.getString(R.string.achievement_halfway_level_2));
				}
				if (score == 100) {
					Games.Achievements
							.unlock(mGoogleApiClient,
									c.getString(R.string.achievement_completed_level_2));
				}

				// Push score to leaderbords
				Games.Leaderboards.submitScore(mGoogleApiClient,
						getString(R.string.leaderboard_level_2), score);
			}

			// Global achievements
			if (isPrime(score)) {
				Games.Achievements.unlock(mGoogleApiClient,
						c.getString(R.string.achievement_prime));
			}

			if (score == 66) {
				Games.Achievements
						.unlock(mGoogleApiClient,
								c.getString(R.string.achievement_omg_u_r_teh_uber_leet));
			}

			// Games Played +1
			Games.Achievements.increment(mGoogleApiClient,
					c.getString(R.string.achievement_or_addicted), 1);

			Games.Achievements.increment(mGoogleApiClient,
					c.getString(R.string.achievement_bored), 1);
