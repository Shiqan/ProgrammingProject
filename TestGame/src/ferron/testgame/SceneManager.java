...
public class SceneManager extends Object
{
	...
	private IUpdateHandler mLoadingScreenHandler = new IUpdateHandler() {
		...
		public void onUpdate(float pSecondsElapsed) {
			...
			mNextScene.mElapsedLoadingScreenTime += pSecondsElapsed;
			...
			if(mNumFramesPassed>1 && mNextScene.mElapsedLoadingScreenTime>=mNextScene.mMinLoadingScreenTime) {
				...
				mNextScene.mElapsedLoadingScreenTime = 0f;
				mNumFramesPassed = -1;
	...
	public OptionsLayer currentLayer;
	...
	public void showScene(final ManagedScene pManagedScene) {
		...
		ApplyingSceneManager.mCamera.set(0f, 0f, ResourceManager.getInstance().cameraWidth, ResourceManager.getInstance().cameraHeight);
		ApplyingSceneManager.mCamera.setBounds(0f, 0f, ResourceManager.getInstance().cameraWidth, ResourceManager.getInstance().cameraHeight);
		// If the new managed scene has a loading screen.
		if(pManagedScene.mHasLoadingScreen) {
	...
	public void showMainMenu() {
		showScene(MenuScene.getInstance());
	}
	...
	public void showLayer(final OptionsLayer optionsLayer, final boolean pSuspendSceneDrawing, final boolean pSuspendSceneUpdates, final boolean pSuspendSceneTouchEvents) {
		...
		if(pSuspendSceneDrawing || pSuspendSceneUpdates || pSuspendSceneTouchEvents) {
			...
			mEngine.getCamera().getHUD().setChildScene(optionsLayer, pSuspendSceneDrawing, pSuspendSceneUpdates, pSuspendSceneTouchEvents);
			...
			mEngine.getCamera().getHUD().setChildScene(optionsLayer);
		}
		// Set the camera for the managed layer so that it binds to the camera if the camera is moved/scaled/rotated.
		optionsLayer.setCamera(mEngine.getCamera());
		// Scale the layer according to screen size.
		optionsLayer.setScale(ResourceManager.getInstance().cameraScaleFactorX, ResourceManager.getInstance().cameraScaleFactorY);
		// Let the layer know that it is being shown.
		optionsLayer.onShowLayer();
		// Reflect that a layer is shown.
		isLayerShown = true;
		// Set the current layer to pLayer.
		currentLayer = optionsLayer;
	}
